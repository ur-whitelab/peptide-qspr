        ...
        for j in range(indices[1]):
            for k in range(indices[2]):
                for a in range(indices[3]):
                    for b in range(indices[4]):
                        for c in range(indices[5]):
                            if(count not in motif_lists.keys()):
                                motif_lists[count] = [''.join(ALPHABET[motif[0][i]]+ ALPHABET[motif[1][j]]+ ALPHABET[motif[2][k]]+ ALPHABET[motif[3][a]]+ ALPHABET[motif[4][b]] + ALPHABET[motif[5][c]])]
                            else:
                                motif_lists[count].append(''.join(ALPHABET[motif[0][i]]+ ALPHABET[motif[1][j]]+ ALPHABET[motif[2][k]]+ ALPHABET[motif[3][a]]+ ALPHABET[motif[4][b]] + ALPHABET[motif[5][c]]))
    count += 1
