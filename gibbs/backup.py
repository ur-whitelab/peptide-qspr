...
def get_tot_prob(peptide, bg_dist,  motif_dists, class_dist=None, motif_class=None, motif_start = None, start_dist = None):
    ...
    assert( not (motif_start is None and start_dist is None))#need some way to find start
    assert( not (motif_class is None and class_dist is None))#need some way to find class
    length = len(peptide)
    prob = 0.0
    #for use during Gibbs steps, when start position is given (sampled)
    if start_dist is None:#use set start position
        if class_dist is None:#use set class value
            for i in range(length):#loop over all AA
                ...
                if(i < motif_start or i >= motif_start + MOTIF_LENGTH):#\geq because of indexing
                    ...
                    prob += motif_dists[motif_class][ i - motif_start][peptide[i]]
        else:#class_dist is not None -> use distros
            for i in range(length):
                if(i < motif_start or i >= motif_start + MOTIF_LENGTH):#not in a motif
                    prob += bg_dist[peptide[i]]
                else:#in a motif
                    for j in range(NUM_MOTIF_CLASSES):#loop over all possible classes
                        prob += motif_dists[j][i - motif_start][peptide[i]] * class_dist[j]
    #for use during evaluation & finding ROC data
    else:#start_dist is not None -> use distros, no set value
        if class_dist is None:#use set class value but draw from start position distro
            for i in range(length):
                for j in range(length - MOTIF_LENGTH):#all possible motif start positions
            ...
            for i in range(length):
                for j in range(length - MOTIF_LENGTH):
                    if(i < j or i >= j+MOTIF_LENGTH):#not in a motif so don't care about class
                        prob += bg_dist[peptide[i]] * start_dist[j]
                    else:#we are in a motif so we ALSO iterate over all possible motif classes
...
apd_data, all_apd_aa  = read_data(DATA_DIR+'APD_qsar_new.txt')
...
for _ in range(NRUNS):
    ...
    for key in apd_data.keys():#loop over lengths of input peptides
        ...
        for i in range(len(apd_data[key])):#loop over all peptides of each length
            ...
            possible_starts = np.array(range(key - MOTIF_LENGTH +1))
        ...
        for i in range(len(motif_start_dists[key])):#loop over all AA in pep
            for j in range(len(motif_start_dists[key][i])):#loop over all possible start positions
                motif_start_dists[key][i][j] += get_tot_prob(
                    apd_data[key][i], bg_dist,  motif_dists, motif_class_dists[key][i],
                    motif_start = j, motif_class=None
                )#would do motif_start_dists[key][i]
        ...
        for i in range(len(motif_class_dists[key])):
            for j in range(NUM_MOTIF_CLASSES):
                motif_class_dists[key][i][j] += get_tot_prob(
                    apd_data[key][i], bg_dist,  motif_dists, class_dist=None, motif_class=j,
                    motif_start = None, start_dist=motif_start_dists[key][i]
                )
        for i in range(len(motif_class_dists[key])):
...
for i in range(NUM_MOTIF_CLASSES):
    for j in range(MOTIF_LENGTH):
        ...
        plt.xticks(range(20), ALPHABET)
        plt.savefig('class_{}_position_{}_motif_dist.png'.format(i,j))
        plt.close(fig)
