        ...
        for i in range(len(motif_class_dists[key])):
            for j in range(len(motif_class_dists[key][i])):
                ...
                motif_class_dists[key][i][j] += get_tot_prob(
                    apd_data[key][i], bg_dist,  motif_dists, class_dist=motif_class_dists[key][i], motif_class=j,
                    motif_start = None, start_dist=(motif_start_dists[key][i])#only use overall 
                )* ETA#/2.0
        for i in range(len(motif_class_dists[key])):
