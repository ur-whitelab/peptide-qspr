...
with aa_model:
    ...
    motif_start_p = pm.Dirichlet('motif_start_p', a=np.ones(max(apd_data.keys())), shape = max(apd_data.keys()))
    motif_start = pm.Categorical('motif_start', p=motif_start_p, shape=max(apd_data.keys()))
...
#    start_trace = pm.sample(len(apd_data.keys()), step = step1)
    ...
    for i in apd_data.keys():
        ...
        single_pep_trace = pm.sample(6000, step=[step1, step2])#, step=pm.Metropolis())#[step1, step2])
        ...
        background_ppc = pm.sample_ppc(background_trace, samples=2000)#resample from bg each time
        for j in range(len(background_ppc['bg_dist'])):
            peps.append(str(background_ppc['bg_dist'][j][:start_loc]) +
                        str(single_pep_ppc['motif_dist_{}'.format(i)][j]) +
                        str(background_ppc['bg_dist'][j][(start_loc + MOTIF_LENGTH):i])
            )
