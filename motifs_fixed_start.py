...
def get_tot_prob(peptide, bg_hist,  motif_hist_dict):
    '''Takes in a single peptide as a LIST OF INTS, the background histogram, and the
       dict of motif histograms, and returns the total probability
    ...
    for i in range(0, length):
        if ( 1<=i and i <=4):
            probs[i] = motif_hist_dict[length][i-1][peptide[i]]
        else:
            probs[i] = get_hist_prob(bg_hist, peptide[i])
...
#    start_trace = pm.sample(len(apd_data.keys()), step = step1)
    ...
    bg_dist = pm.Categorical('bg_dist', p = bg_p,
                             shape=max(apd_data.keys()), observed=all_apd_aa)
    print('Now sampling for background distribution')
    step0 = pm.CategoricalGibbsMetropolis(vars=[bg_dist])
    background_trace = pm.sample(6000, step=[step0])

    for i in apd_data.keys():
        motif_start = FIXED_START_POS%(i-MOTIF_LENGTH) #fixed index
            

        #build the observation array
        obs = [apd_data[i][j][(motif_start):MOTIF_LENGTH+motif_start] for j in range(len(apd_data[i]))]
        motif_dist = pm.Categorical('motif_dist_{}'.format(i), p = motif_p,
                                    shape = MOTIF_LENGTH,
                                    observed = obs
                                   )
        #using the bg distro, get a good sampling
        step2 = pm.CategoricalGibbsMetropolis(vars=[motif_dist])
        single_pep_trace = pm.sample(3000, step=[ step2])#, step=pm.Metropolis())#[step1, step2])
        #sample one whole peptide, once for each count of the given length.
        single_pep_ppc = pm.sample_ppc(single_pep_trace, samples=1000)#samples=len(apd_data[i]))
        background_ppc = pm.sample_ppc(background_trace, samples=1000)#resample bg ppc each time
        for j in range(len(background_ppc['bg_dist'])):
            peps.append(str(background_ppc['bg_dist'][j][:motif_start]) +
                        str(single_pep_ppc['motif_dist_{}'.format(i)][j]) +
                        str(background_ppc['bg_dist'][j][(motif_start + MOTIF_LENGTH):i])
            )
...
bg_histogram = np.histogram(single_pep_ppc['bg_dist'], bins=range(21), normed=True)
...
fake_probs = [get_tot_prob(item, bg_histogram,  motif_histograms) for item in fake_intpeps]
apd_probs = [ get_tot_prob(item, bg_histogram,  motif_histograms) for item in intpeps]

rocmin = min(min(fake_probs), min(apd_probs))
rocmax = max(max(fake_probs), max(apd_probs))
npoints = 200
plotname = 'data/motif_ROC_fixed_start_2.png'
